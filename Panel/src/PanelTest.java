...
class PanelTest {
	...
	public void panelSizes() {
		Assert.assertEquals(3, Range.range(0, 2).size());
		Assert.assertEquals(1, Range.range(2, 2).size());
		Assert.assertEquals(5, Range.range(-2, 2).size());
	}
	...
	public void panelIterator() {
		...
		for(int i=0; i<1000; i++) {
			...
			List<Integer> panel = Range.range(start, start + 99);
	...
	public void panelIterator2() {
		...
		for(int i=0; i<1000; i++) {
			int start = random.nextInt();
			List<Integer> panel = Range.range(start, start + 73);
